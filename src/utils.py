...
def apply_affine(verts, A):
    ...
    #transformed_verts = torch.matmul(A, verts.T)
...
def sample_bary(faces, vertices):
    ...
    for f in range(num_faces):
        ...
        #new_vertices[f] = torch.matmul(bary[:, f], face_verts[:, f])
...
def add_vertices(mesh):
    ...
    for f in range(num_faces):
        ...
        if face_uvs is not None:
            ...
            #new_uvs[f] = torch.matmul(bary[:, f], face_uvs[:, f])
...
def get_texture_visual(res, nt, mesh):
    ...
    for face in range(num_faces):
        ...
        #inside_triangle_uv = torch.matmul(inside_triangle, uv_map[face])
        inside_triangle_xyz = inside_triangle @ faces_vt[face]
        #inside_triangle_xyz = torch.matmul(inside_triangle, faces_vt[face])
...
def extract_from_gl_viewmat(gl_mat):
    ...
    #camera_location = torch.matmul(np.array([-s, -u, f]).T, coord)
...
class FourierFeatureTransform(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        #res = torch.matmul(x, self._B.to(x.device))
