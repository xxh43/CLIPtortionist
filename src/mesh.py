...
class Mesh():
    def __init__(self, vertices, faces, color=torch.tensor([0.0,0.0,1.0])):
        ...
        self.vertices = torch.tensor(vertices, device=device, dtype=torch.float)
        self.faces = torch.tensor(faces, device=device, dtype=torch.int64)
    ...
    def update_normals(self):
        ...
        temp_mesh = trimesh.Trimesh(to_numpy(self.vertices), to_numpy(self.faces), process=False)
        #temp_mesh.fill_holes()
        temp_mesh.fix_normals()

        self.vertices = torch.tensor(temp_mesh.vertices, device=device, dtype=torch.float)
        self.faces = torch.tensor(temp_mesh.faces, device=device, dtype=torch.long)
        self.face_normals = temp_mesh.face_normals

        vertex_normals = trimesh.geometry.mean_vertex_normals(len(self.vertices), to_numpy(self.faces), temp_mesh.face_normals)    
        #print('vertex_normals shape', vertex_normals.shape)
        self.vertex_normals = torch.tensor(vertex_normals, device=device, dtype=torch.float)
        self.vertex_normals = self.vertex_normals/torch.norm(self.vertex_normals, dim=1).unsqueeze(dim=1)
